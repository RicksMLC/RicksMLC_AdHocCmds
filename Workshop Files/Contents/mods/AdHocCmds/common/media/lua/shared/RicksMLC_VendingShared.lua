...
function RicksMLC_VendingMachineConfig:new(prevConfig, cashIns, prevLevel)
    ...
    o.cashIns = cashIns -- Note that the default (first) config has no cashIns. When this is nil, no pop is done.
...
function RicksMLC_VendingMachineConfig:UpdateAttribute(vendingConfigFile, attrib, attribName, delim)
    ...
    while attribLine do
        local attribList = RicksMLC_SharedUtils.SplitStr(attribLine, delim)
        attrib[i] = attribList
        i = i + 1
...
local function reportVendingError(errorMsg)
    --RicksMLC_SharedUtils.Think(getPlayer(), errorMsg, 3) -- Error message is red
...
function RicksMLC_VendingMachineConfig:AddContainerPrizes(vendingConfigFile)
    ...
    local attribLine = vendingConfigFile:Get(attribName .. tostring(i))
    while attribLine do
        local attribList = RicksMLC_SharedUtils.SplitStr(attribLine, ",")
...
function RicksMLC_VendingMachineConfig:Update(vendingConfigFile)
    ...
    self.configFile = vendingConfigFile
end
...
--             --prize = InventoryItemFactory.CreateItem(prizeType) -- B42 change:
--             prize = instanceItem(prizeType)
--             if not prize then
...
--         sendClientCommand(getPlayer(), "RicksMLC_AdHocCmdsServer", "VendingConfigRequest", {})
--     else
...
--Events.OnClientCommand.Add(RicksMLC_Vending.OnServerCommand)